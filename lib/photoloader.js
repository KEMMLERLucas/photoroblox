...
async function next(nbPage, taille) {
    let res = await fetch(`https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api/photos/?page=${nbPage+1}&size=${taille}`, {credentials: "include"});
    if (res.ok){
        let photos = await res.json();
        console.log(photos);
        return photos;
...
async function loadRessource(uri) {
    let res = await fetch(`https://webetu.iutnc.univ-lorraine.fr/${uri}`, {credentials: "include"});
    if (res.ok){
        let photos = await res.json();
        return photos;
    }
